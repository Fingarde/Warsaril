...
public class RemoveWarp implements CommandExecutor
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
    {
        ...
        if(!player.hasPermission("warsaril.removeWarp")) { player.sendMessage(Main.noPermission(label)); return false;}
        ...
        try {
            ...
            {
                player.sendMessage(Main.prefix + ChatColor.RED + "Il n\'y a aucun warp de ce nom");
                return false;
            }

            sql.prepareStatement("DELETE FROM warps WHERE type = 'warp' AND name = '" + name + "';").execute();

            player.sendMessage(Main.prefix + ChatColor.GREEN + "Le warp " + ChatColor.YELLOW + name + ChatColor.GREEN + " a été supprimé");
            Main.getLog().warning( player.getName() + " a supprimé le warp " + name);
